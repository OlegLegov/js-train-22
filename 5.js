...
class Writer {
  ...
  #content = ""; // Приватна властивість для збереження поточного тексту

  /**
   * Сетер для властивості content. Приймає значення newContent (новий текст),
   * який встановлює як поточний текст. Викликає метод #store() для збереження поточного стану тексту.
   */
  ...
  /**
   * Гетер для властивості content. Повертає поточний текст.
   */
  get content() {
    return this.#content;
  }
  ...
  /**
   * Приватний метод #store використовується для зберігання поточного стану тексту.
   * Викликає статичний метод класу Version, create, передаючи поточний текст як аргумент.
   */
  #store() {
  ...
  /**
   * Метод restore відновлює попередній стан тексту, викликаючи статичний метод класу Version, restore.
   * Повертає останню збережену версію тексту і встановлює її як поточний текст.
   */
  restore() {
    const restoredContent = Version.restore();
...
class Version {
  ...
  // Приватний статичний масив для зберігання всіх версій
  static #versions = [];
   ...
   * Створений екземпляр додається до масиву версій.
   */
  ...
  static restore() {
    ...
    Version.#versions.pop(); // Видаляємо останню версію
    return Version.#versions.length > 0
      ? Version.#versions[Version.#versions.length - 1].content
      : "";
  }
}
